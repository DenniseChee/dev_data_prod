...
shinyUI(
  navbarPage("LEGO Set Visualization", 
       ...
       tabPanel(p(icon("search"), "LookUp on Brickset Website"),
                mainPanel(
                  ...
                  h4("Step 1. Please type the Set ID below and press the 'Go to the Site' button:"),
                  textInput(inputId="setid", label = "Input Set ID"),
                  actionButton("goButtonAdd", "Go to the Site"),
                  h5('Output Address:'),
                  textOutput("address"),
                  p(""),
                  h4("Step 2. Please click the link below to go to the Set's page."),
                  p(""),
                  htmlOutput("inc")
                  )         
                ),
